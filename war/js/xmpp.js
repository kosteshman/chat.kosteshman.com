	...
	setTimeout(function(){
		$.get('/getstatus', {'tratata':Math.round(Math.random(100000)),'referrer':ref},  function(status){
			...
			case 'offline':
				...
				typer.type('Или оставьте мне сообщение прямо здесь - я прочитаю его, когда доберусь до почты.');
				typer.type('Так что не забудьте оставить свои контакты, чтобы я смог вам ответить :)');
				break;
